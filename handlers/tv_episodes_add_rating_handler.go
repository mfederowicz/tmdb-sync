...
// Handle rates a TV episode on behalf of the session's account.
func (h TVEpisodesAddRatingHandler) Handle(ctx context.Context, client *internal.Client) (any, error) {
	status, _, err := client.TVEpisodes.AddRating(ctx, h.SeriesID, h.SeasonNumber, h.EpisodeNumber, h.SessionID, h.Value)
	if err != nil {
		return nil, err
	}
	return status, nil
