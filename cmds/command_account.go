...
func execAccount(fs afero.Fs, client *internal.Client, config *cfg.Config, options *str.Options, args []string) error {
	...
	action := flagSet.String("a", "", "action: details, add-watchlist, add-favorite, favorite-movies, favorite-tv (required)")
	...
	pagesLimit := flagSet.Int("pages-limit", config.PagesLimit, "pages limit, used by -a favorite-movies, favorite-tv (default: pages_limit from config, 0 = unlimited)")
	...
	switch *action {
	case "":
		return fmt.Errorf("account: -a is required (action: details, add-watchlist, add-favorite, favorite-movies, favorite-tv)")
	case "details":
