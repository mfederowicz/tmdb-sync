...
var accountV4Actions = []string{"lists", "favorite-movies", "favorite-tv"}
...
func execAccountAttempt(fs afero.Fs, client *internal.Client, config *cfg.Config, options *str.Options, args []string, retried bool) error {
	...
	v4 := flagSet.Bool("v4", false, "use the v4 API: needs `auth -v4 -a login` first; actions: lists, favorite-movies, favorite-tv")
