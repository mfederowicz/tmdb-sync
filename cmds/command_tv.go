...
const tvActionsHelp = "details, account-states, aggregate-credits, alternative-titles, content-ratings, credits, episode-groups, external-ids, images, keywords, latest, lists, popular, recommendations, reviews, screened-theatrically, similar, translations, videos, watch-providers"
...
func execTVAttempt(fs afero.Fs, client *internal.Client, config *cfg.Config, options *str.Options, args []string, retried bool) error {
	...
	pagesLimit := flagSet.Int("pages-limit", config.PagesLimit, "pages limit, used by -a lists, popular, recommendations, reviews, similar (default: pages_limit from config, 0 = unlimited)")
