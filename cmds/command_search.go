...
func execSearch(fs afero.Fs, client *internal.Client, config *cfg.Config, _ *str.Options, args []string) error {
	...
	action := flagSet.String("a", "", "action: collections, companies, keywords, movies (required)")
	...
	pagesLimit := flagSet.Int("pages-limit", config.PagesLimit, "pages limit, used by -a collections, companies, keywords, movies (default: pages_limit from config, 0 = unlimited)")
	...
	switch *action {
	case "":
		return fmt.Errorf("search: -a is required (action: collections, companies, keywords, movies)")
	case "collections":
		if *query == "" {
