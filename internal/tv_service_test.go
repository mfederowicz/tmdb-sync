...
func TestGetTopRatedTV(t *testing.T) {
	...
	mux.HandleFunc("/tv/top_rated", func(w http.ResponseWriter, r *http.Request) {
		pageNum, err := strconv.Atoi(r.URL.Query().Get("page"))
		if err != nil {
			pageNum = 1
		}
	...
	shows, err := client.TV.GetTopRatedTV(context.Background(), 0)
	if err != nil {
		t.Fatalf("GetTopRatedTV() error = %v", err)
	}
	if len(shows) != 2 {
		t.Fatalf("len(shows) = %d, want 2", len(shows))
	}
...
func TestGetOnTheAirTV(t *testing.T) {
	...
	mux.HandleFunc("/tv/on_the_air", func(w http.ResponseWriter, r *http.Request) {
		pageNum, err := strconv.Atoi(r.URL.Query().Get("page"))
		if err != nil {
			pageNum = 1
		}
	...
	shows, err := client.TV.GetOnTheAirTV(context.Background(), 0)
	if err != nil {
		t.Fatalf("GetOnTheAirTV() error = %v", err)
	}
	if len(shows) != 2 {
		t.Fatalf("len(shows) = %d, want 2", len(shows))
	}
